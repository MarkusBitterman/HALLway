...
- There is no system SSH host key on Android; `~/.ssh/id_ed25519` is the sops identity
...
#### Step 2: Add phone SSH key as sops recipient
...
Add to `.sops.yaml` as `hellomoto` key, update creation_rules for HelloMoto.

Then rekey from 2600AD:

```bash
nix develop
sops updatekeys hosts/HelloMoto/secrets.yaml
```

#### Step 3: Create the GitHub SSH secret

```bash
# From 2600AD dev shell:
sops hosts/HelloMoto/secrets.yaml
```
...
| `DESKTOP_IP_OR_HOSTNAME` | `home/user.nix` | 2600AD LAN IP or WireGuard IP (`10.23.11.80`) |
...
2. Converted to age format and added to `.sops.yaml`
3. Secrets rekeyed with `sops updatekeys`
